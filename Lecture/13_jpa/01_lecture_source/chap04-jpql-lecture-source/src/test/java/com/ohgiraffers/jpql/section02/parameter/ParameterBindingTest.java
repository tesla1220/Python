...
public class ParameterBindingTest {
    ...
    /* 필기 : [파라미터 바인딩]
    *   우리가 만든 변수를 query 문에 적용하기
    *       1. 이름 기준 파라미터(named parameters)
    *           콜론(:) 다음에 이름(menuName) 기준으로 파라미터를 작성한다.
    *       2. 위치 기준 파라미터(positional parameters)
    *           물음표(?) 다음에 값을 설정하고, 위치값은 1부터 시작한다.  */
    ...
    void testParameterByName() {
        ...
        // when
        repository.selectMenuByBindingName(menuName);
        ...
        System.out.println(" menuName : " + menuName + menuList);
