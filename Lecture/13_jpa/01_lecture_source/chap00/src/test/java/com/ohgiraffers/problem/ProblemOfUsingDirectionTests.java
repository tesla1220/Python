...
public class ProblemOfUsingDirectionTests {
    ...
    /*우리가 작성한 테스트 메소드가 실행하기 이전에 한번씩 동작을 할 수 있는 annotation*/
    @BeforeEach
    ...
    void testDirectSQL() throws SQLException {
        ...
        /* 필기. Assertions 클래스는 우리가 작성한 테스트 코드의 검증을 할 수 있는 기능(메소드)을
        * 제공해주는 Class 이다. */
    ...
    void testInsertSQL() throws SQLException {
        ...
        // given : 조건을 넘겨줄 ㄸㅐ. 내가 이 값으로 테스트를 진행할거야.
        Menu menu = new Menu();
        ...
        // then
        /* equals : equals 메소드는 실제값과 기대하는 값의 일치 여부를 동일성으로 판단한다. */
    ...
    /* 필기.
    *   고객, 즉 클라이언트 측에 요구사항 변화로 인한 DB 수정, SQL 문 수정, Application 수정 등의 문제 발생 */
    ...
    /* 필기.
    *   객체 지행 언어의 상속과 유사한 것이 데이터베이스의 서브타입 엔티티 이다.
    *   유사한 것 같지만 다른 부분은 데이터베이스의 상속은 상속 개념을 데이터로 추상화해서
    *   슈퍼타입과 서브타입으로 구분하고, 물리적으로는 다른 테이블로 분리가 된 형태이다.
    *   ( 데이터베이스 관점에서 완벽한 상속을 이뤄내기 어려움 ) */

    /* 3-1. 연관관계 문제 */

    /* 객체지향에서 말하는 1대 1관계, 1대 다 관계는 데이터베이스 저장 구조와 다른 형태이다. */
