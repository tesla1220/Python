...
public class ResolverController {
    ...
    public String stringReturning(Model model) {
        ...
        /* 필기.
        *   문자열로 뷰 이름을 반환한다는 것은
        *   반환 후 ThymeleafViewResolver 에게 네가 앞에 resources/template 붙이고
    ...
    public String stringRedirect() {
        ...
        /* 접두사로 redirect: 을 하면
        forward 방식이 아닌 redirect 를 시켜준다.
        */
        return "redirect:/";
    }

    /* 필기.
    *   redirect 가 발생한다 -> 재요청이 발생하기 때문에 request 의 scope 는 소멸된다.
    *   하지만 스프링에서는 RedirectAttributes 타입을 통해
    *   redirect 시 속성 값을 저장할 수 있도록 하는 기능을 제공한다.
    * */
    ...
    public String stringRedirectFlashAttr(RedirectAttributes rttr) {
        ...
        *   session 에 동일한 키값이 존재하면 안된다. */
        rttr.addFlashAttribute("flashMessage", "리다이렉트 attr 사용해서 redirect");
    ...
    public ModelAndView modelAndViewReturning (ModelAndView mv) {
        ...
        /* 필기.
        *   모델(값 저장) 과 뷰를 합친 개념
        *   핸들러 어댑터가 핸들러 메소드를 호출하고 반환받은 문자열을 ModelAndView 로 만들어서
        *   dispatcherServlet 에 반환한다.
